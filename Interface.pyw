...
class Window(QMainWindow):
    def __init__(self) -> None:
        ...
        autoPicking.setStatusTip(
            'Automatically pick the fisrt arrival for each signals ().')
        ...
        if not (self.dataUI.animationPicking.notPicking):
            ...
            if event.button == MouseButton.LEFT and ((time.time() - self.dataUI.animationPicking.timeOnClick) < self.dataUI.animationPicking.maxClickLength):
                if self.buttonTabPickingSetT0.isChecked():
                    ...
                    # To remove a picked trace, click on times below 0
                    if self.dataUI.animationPicking.mousePosition[0] < self.dataUI.beginTime[self.dataUI.sisFileId]:
                        ...
                        self.dataUI.pickingError[self.dataUI.sisFileId,
                                                 self.dataUI.animationPicking.currSelect] = np.nan
                        QMessageBox.warning(
                            self, 'Warning!', 'Negative time are not physically possible!')
                    else:
                        self.dataUI.picking[self.dataUI.sisFileId,
    ...
    def animationZoom(self, i):
        ...
        timeSEG2 = np.arange(self.dataUI.beginTime[self.dataUI.sisFileId],
                             self.dataUI.beginTime[self.dataUI.sisFileId]+nbPoints*deltaT, deltaT)
        ...
        if not (np.isnan(currPicking)):
            currError = self.dataUI.pickingError[self.dataUI.sisFileId,
                                                 self.dataUI.animationPicking.currSelect] * currPicking
    ...
    def animationMain(self, i):
        ...
        if not (self.dataUI.animationPicking.notPicking) or self.dataUI.animationPicking.changedSelect:
            ...
            for tr in self.dataUI.sisData[self.dataUI.sisFileId]:
                data = tr.data
                if not (self.dataUI.animationPicking.first):
                    data = data/(max(data[np.logical_and(timeSEG2 > limitsX[0], timeSEG2 < limitsX[1])])-min(
                        data[np.logical_and(timeSEG2 > limitsX[0], timeSEG2 < limitsX[1])]))+i
                else:
                ...
                if not (np.isnan(currPicking)):
                    ...
                    axMain.plot([currPicking, currPicking],
                                [i-0.5, i+0.5], color='g')
                    axMain.plot(
                        [currPicking - currError, currPicking - currError], [i-0.25, i+0.25], ':g')
                    axMain.plot(
                        [currPicking + currError, currPicking + currError], [i-0.25, i+0.25], ':g')
                i += 1
        ...
        if AIC_SIMPLE_IMPORT:
            ...
            for i, st in enumerate(self.dataUI.sisData):
                for j, tr in enumerate(st):
                    ...
                    # max(self.dataUI.picking[i, j] * 0.05, 0.000001)
                    self.dataUI.pickingError[i, j] = 0.05
        ...
        for nbFile in range(len(self.dataUI.paths.seg2Files)):
            for i in range(len(self.dataUI.sisData[0])):
                ...
                if sId != rId:  # The traveltime for source = receiever is 0 and not usefull for inversion!
                    ...
                    # max([self.dataUI.pickingError[nbFile, i] * t, 0.000001])# Avoid the error with null error in inversion
                    err = self.dataUI.pickingError[nbFile, i]
                    if not (np.isnan(t)):
    ...
    def _initPygimli(self, fname):
        ...
        self.dataUI.invData.mesh = self.dataUI.invData.manager.createMesh(
            data=self.dataUI.invData.data, paraMaxCellSize=self.dataUI.invData.meshMaxCellSize, paraDepth=self.dataUI.invData.meshDepthMax)
    ...
    def _runInversion(self):
        ...
        if self.dataUI.invData.data is not None:
            ...
            self.invModelGraph.axes.set_title('Inversion result (chi² = {: .2f}, RMS = {: .2f} %, RRMS = {: .2f} %)'.format(
                self.dataUI.invData.manager.inv.chi2(), self.dataUI.invData.manager.inv.absrms()*100, self.dataUI.invData.manager.inv.relrms()*100))
